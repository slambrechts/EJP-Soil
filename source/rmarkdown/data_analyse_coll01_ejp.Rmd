...
```{r per-family-collembola-rarefied}
...
Per genus collembola rarefied

```{r per-genus-collembola-rarefied}
tax_table(physeq) %>%
  as.data.frame() %>%
  group_by(Genus) %>%
  summarise(OTU_Count = n()) %>%
...
Per genus collembola rarefied known genera

```{r per-known-genus-collembola-rarefied}
...
```{r per-species-collembola-rarefied}
tax_table(physeq) %>%
  as.data.frame() %>%
  group_by(Species) %>%
  summarise(OTU_Count = n()) %>%
...
```{r per-known-species-collembola-rarefied}
...
```{r read-counts-collembola-subset-order}
tidy_physeq_rarefied %>%
...
```{r read-counts-collembola-subset-known-order}
...
```{r read-counts-collembola-subset-family}
tidy_physeq_rarefied %>%
...
```{r read-counts-collembola-subset-known-family}
...
We hebben de ordinaties reeds apart gedaan dacht ik

```{r ordination-phyloseq, eval=FALSE}
