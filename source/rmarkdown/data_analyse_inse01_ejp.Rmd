...
```
```{r Species-detected-with-INBO-LUCAS }
...
# Extract species detected with INBO_0_10 but not LUCAS for each location
...
for (loc in locations) {
  ...
  otu_ids_inbo <- rownames(otu_table(physeq_loc)[, which(as.logical(apply(otu_table(physeq_loc), 2, any)) & sample_data(physeq_loc)$Protocol2 == "INBO_0_10"), drop = FALSE])

# Get OTU IDs detected using LUCAS method
  otu_ids_lucas <- rownames(otu_table(physeq_loc)[, which(as.logical(apply(otu_table(physeq_loc), 2, any)) & sample_data(physeq_loc)$Protocol2 == "Lucas"), drop = FALSE])



  # Check if otu_ids_inbo and otu_ids_lucas have the same OTU IDs
  if (identical(otu_ids_inbo, otu_ids_lucas)) {
    cat("Location:", loc, "\n")
    ...
    otu_ids_diff <- setdiff(otu_ids_inbo, otu_ids_lucas)
    
    # Get Species names for the differential OTUs
    species_names <- levels(tax_table(physeq_loc)[match(otu_ids_diff, rownames(tax_table(physeq_loc))), "species"])
    species_names <- paste(species_names, collapse = ", ")
    
    # Print the result
    cat("Location:", loc, "\n")
    cat("Species detected using INBO_0_10 but not LUCAS:", species_names, "\n\n")
  }
}
