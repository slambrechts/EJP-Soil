...
ordmat <- psotu2veg(physeq_full_rarefied)
...
samples <- sample_data(physeq_full_rarefied) %>%
  as.data.frame() %>%
  as_tibble()
...
clr_physeq_rarefied <- transform(physeq_full_rarefied, "clr")

# Calculate Bray distance matrix based on the CLR-transformed data
bray_distance <- distance(clr_physeq_rarefied, method = "bray")
