...
# Your original phyloseq object (replace with your actual object)
phyloseq_species_venn <- physeq_srs_known_species

print(phyloseq_species_venn)

# Extract sample data
sample_data <- as.data.frame(sample_data(phyloseq_species_venn))
...
# Remove samples
phyloseq_species_venn_filtered <- subset_samples_no_zero(phyloseq_species_venn, !(Name %in% samples_to_remove))
phyloseq_species_venn_filtered <- subset_samples_no_zero(phyloseq_species_venn_filtered, Layer != "litter")


# Make venn_diagram per
# Per locatie
phyloseq_species_venn__filtered_location <- phyloseq_species_venn_filtered

# List of unique locations
locations <- unique(sample_data(phyloseq_species_venn__filtered_location)$Location)
...
for (loc in locations) {
  # Subset the phyloseq object for the current location
  phyloseq_sub <- subset_samples_no_zero(phyloseq_species_venn__filtered_location, Location == loc)
...
# Subset only known species
physeq_srs_known_species # KAN IK HIERMEE WERKEN???

otu_table <- otu_table(physeq_srs_known_species)
head(otu_table)
tax_table <- tax_table(physeq_srs_known_species)
head(tax_table)
sample_data <- sample_data(physeq_srs_known_species)
head(sample_data)

unique(sample_data(physeq_srs_known_species)$Protocol2)

locations <- unique(sample_data(physeq_srs_known_species)$Location)

# Extract locations with both INBO and Lucas protocols
locations_with_lucas <- unique(sample_data(physeq_srs_known_species)$Location[sample_data(physeq_srs_known_species)$Protocol2 == "Lucas"])
...
for (loc in locations) {
  ...
  physeq_loc <- subset_samples(physeq_srs_known_species, Location == loc)
...
otu_table <- otu_table(physeq_srs_known_genera)
head(otu_table)
tax_table <- tax_table(physeq_srs_known_genera)
head(tax_table)
sample_data <- sample_data(physeq_srs_known_genera)
head(sample_data)

unique(sample_data(physeq_srs_known_genera)$Protocol2)

locations <- unique(sample_data(physeq_srs_known_genera)$Location)

# Extract locations with both INBO and Lucas protocols
locations_with_lucas <- unique(sample_data(physeq_srs_known_genera)$Location[sample_data(physeq_srs_known_genera)$Protocol2 == "Lucas"])
...
for (loc in locations) {
  ...
  # Subset phyloseq object for the current location
  physeq_loc <- subset_samples(physeq_srs_known_genera, Location == loc)
...
# Get OTU IDs detected using LUCAS method
  ...
  species_names_inbo <- tax_table(physeq_loc)[otu_ids_inbo, "Genus"]
...
# Get OTU IDs detected using LUCAS method
  ...
  if (length(otu_ids_inbo) == 0) {
  # If no OTU IDs detected using INBO_0_10 method
  genus_names_inbo <- character(0)  # Or any appropriate placeholder
  }
...
# Find genus detected with LUCAS but not INBO_0_10
  ...
  if (length(genus_lucas_not_inbo) == 0) {
    ...
    cat("Location:", loc, "\n")
    cat("Species detected using LUCAS but not INBO_0_10:", paste(genus_lucas_not_inbo, collapse = ", "), "\n\n")
  }
}
...
# Get OTU IDs detected using LUCAS method
  ...
  if (length(otu_ids_lucas) == 0) {
  # If no OTU IDs detected using LUCAS method
  species_names_lucas <- character(0)
  } else {
  # Get genus names corresponding to OTU IDs detected using LUCAS method
  genus_names_lucas <- tax_table(physeq_loc)[otu_ids_lucas, "Genus"]
  }
...
form_rarefaction <- bf(
  ...
  + Protocol2
  + Land_use:Protocol2
  + (1 | Name) # Moet dit Name zijn zoals bij Inse01?
)
