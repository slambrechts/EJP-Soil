...
```{r}
tidy_physeq$samples %>%
  distinct(Cmon_PlotID, c_LB72X, c_LB72Y, Landgebruik_ejp) %>%
  st_as_sf(coords = c("c_LB72X", "c_LB72Y"), crs = 31370) %>%
  ggplot() +
  geom_sf(aes(colour = Landgebruik_ejp))
```
...
```{r}
tidy_physeq_rarefied %>%
  tidytacos::tacoplot_venn(Landgebruik_ejp)
```
...
combined_column <- paste(sample_data(physeq_rarefied)$Landgebruik_ejp, sample_data(physeq_rarefied)$Diepte, sep = "_")

# Add the new combined column to the sample metadata with the desired name
sample_data(physeq_rarefied)$Landgebruik_ejp_diepte <- combined_column
...
if (system(command = "which ktImportText",  intern = FALSE, ignore.stderr = TRUE,  ignore.stdout = TRUE) != 1) {
  psadd::plot_krona(physeq_rarefied, "ejp_Olig01_Annelida_rar_species_alle_stalen_Landgebruik_ejp_per_diepte", "Landgebruik_ejp_diepte", trim = T)
}
...
```{r}
tidy_physeq_rarefied %>%
  tidytacos::tacoplot_stack(x = Landgebruik_ejp)
```
...
marginaleffects::plot_predictions(
  m_annelida_richness,
  condition = c("Landgebruik_ejp", "Diepte"),
  vcov = vcov(m_annelida_richness),
  re.form = NA,
  type = "response") +
...
true_rarefaction_results <- cbind(
  sam_new %>% select(Cmon_PlotID, Diepte, Landgebruik_ejp),
...
conditional_effects(
  m_rarefaction,
    effects = c("Landgebruik_ejp:Diepte"),
  re.form = NA)
```
...
plot_ordination(physeq, pcoa_result, type = "samples",
                color = "Landgebruik_ejp", shape = "Diepte") +
  theme_minimal()
```
